...
async function boot() {
  ...
  // A registrar is CONFIGURED, so the flow below is the right thing to show — but it is not usable until the probe
  // comes back and the handlers are attached. An enabled button with no listener is a lie the page tells for as long
  // as the probe takes, so it stays disabled and says what it is doing.
  const mintBtn = $("#g-mint");
  if (mintBtn) { mintBtn.disabled = true; mintBtn.textContent = "Checking the registrar…"; }
  const ready = () => { if (mintBtn) { mintBtn.disabled = false; mintBtn.textContent = "Mint my passport →"; } };
  let health;
  ...
  try { const a = await (await fetchT(REG + "/accreditation")).json(); anchors = { [REGID]: { issuer_key: a.issuer_key, log_root_key: a.log_root_key } }; } catch (e) { return unavailable(`Couldn't read the registrar's accreditation — ${why(e, `<code>${REG}</code>`)}.`); }
  ...
  ready(); // handlers attached — only now is the button honest
...
async function mint() {
  ...
  try {
    rec = await (await fetchT(REG + "/demo/issue", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ operator: $("#g-op").value, lineage: $("#g-lin").value }) })).json();
...
async function verify() {
  ...
  try {
    const pres = await (await fetchT(`${REG}/present?sub=${encodeURIComponent(rec.sub)}&now=${NOW}`)).json();
...
async function _revoke() {
  ...
  try {
    const rv = await (await fetchT(REG + "/demo/revoke", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ sub: rec.sub, now: NOW }) })).json();
